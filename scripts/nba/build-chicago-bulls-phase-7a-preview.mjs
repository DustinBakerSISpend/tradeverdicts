...
assert(reviewed.result === "PASS" && reviewed.phase === "7A", "Invalid reviewed source.");
assert(Array.isArray(reviewed.records) && reviewed.records.length === 219, "Reviewed-row count drifted.");
...
assert(candidates.length === 219, "Candidate queue count drifted.");
assert(routing.length === 15, "Routing queue count drifted.");
assert(candidates.filter((record) => record.mergeExclude).length === 7, "Merge queue count drifted.");
...
const files = {
  candidateQueue: "chicago-bulls-phase-7a-candidate-queue.csv",
  crossTeamPreview: "chicago-bulls-phase-7a-cross-team-match-preview.csv",
  routingQueue: "chicago-bulls-phase-7a-routing-queue.csv",
  previewJson: "chicago-bulls-phase-7a-preview.json",
};
