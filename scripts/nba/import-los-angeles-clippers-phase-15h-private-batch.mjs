...
function perspectiveGrades(record, partner) {
  const grades = {};
  const clippers = clean(record["Clippers Grade"]);
  const aggregate = clean(record["Partner Aggregate Grade"]);
  if (clippers) grades["los-angeles-clippers"] = clippers;
  if (aggregate) {
...
function clippersPerspective(record, teams) {
  const partner = teams.find((team) => team !== "los-angeles-clippers") ?? null;
  return {
    sourceTeam: "los-angeles-clippers",
    sourceBatchId: "los-angeles-clippers-phase-15h",
    sourceTradeId: clean(record["Trade ID"]),
    sourcePerspectiveKey: `los-angeles-clippers:${clean(record["Trade ID"])}`,
...
function appendClippersPerspective(existingTrade, record, importedAt) {
  const protectedBefore = JSON.stringify(immutableTradeProjection(existingTrade));
  assert(
    sourcePerspectiveCount(existingTrade, "los-angeles-clippers") === 0,
    `${record["Trade ID"]}: Clippers perspective already exists.`,
  );
  const teams = uniqueSorted(existingTrade.teams ?? []);
  const perspective = clippersPerspective(record, teams);
  let perspectives;
  if (Array.isArray(existingTrade.perspectives)) {
    ...
    perspectives = {
      ...
      "los-angeles-clippers": {
        sourceSubmissionId: `los-angeles-clippers-phase-15h-${clean(record["Trade ID"])}`,
        sourceTradeId: clean(record["Trade ID"]),
        editorialStatus: "private-imported-los-angeles-clippers-phase-15h",
        grade: clean(record["Clippers Grade"]),
        verdict: clean(record["Final Verdict"]),
        summary: clean(record["Final Trade Summary"]),
        analysis: clean(record["Final Trade Analysis"]),
  ...
  const clippersGrade = clean(record["Clippers Grade"]);
  const partnerGrade = clean(record["Partner Aggregate Grade"]);
  if (clippersGrade) mergedGrades["los-angeles-clippers"] = clippersGrade;
  ...
  const updated = {
    ...
    perspectiveReconciliations: [
        ...
        : []),
      {
        sourceBatchId: "los-angeles-clippers-phase-15h",
        sourceTradeId: clean(record["Trade ID"]),
        packageId: `los-angeles-clippers-phase-15h-${clean(record["Trade ID"])}`,
  ...
  assert(
    sourcePerspectiveCount(updated, "los-angeles-clippers") === 1,
    `${record["Trade ID"]}: Clippers perspective append count drifted.`,
  );
  return updated;
}
function canonicalIdForSource(sourceTradeId) {
  const match = clean(sourceTradeId).match(/^LAC-(\d{4})-(\d{4})$/u);
  assert(match, `Invalid Clippers source Trade ID: ${sourceTradeId}`);
  return `nba-trade-lac-${match[1]}-${match[2]}`;
}
function makeAssetId(sourceTradeId, side, assetIndex, rawAsset, fromTeam, toTeam) {
  return `phase15h-asset-${sha256(
...
function buildNewTrade({ packageRecord, record, dependencyRows, relationshipRows, importedAt, playerMap }) {
  ...
  const sourcePartner = clean(record["Counterpart Slugs"]);
  const partner = canonicalTeamSlug(sourcePartner);
  assert(partner && !partner.includes(","), `${sourceTradeId}: ready package must have exactly one counterpart slug.`);
  const teams = uniqueSorted(["los-angeles-clippers", partner]);
  ...
  const assets = dependencyRows
    ...
    .map((dependency) => {
      ...
      const fromTeam = side === "received" ? partner : "los-angeles-clippers";
      const toTeam = side === "received" ? "los-angeles-clippers" : partner;
      ...
      const asset = {
        ...
        direction: side,
        sourceTeam: "los-angeles-clippers",
        edgeClass: "clippers-source-route",
        ...
        privateOnly: true,
        previewOnly: false,
        auditStatus: "private-imported-los-angeles-clippers-phase-15h",
      };
      if (linked.length > 0) {
  ...
  if (sources.length === 0) {
    sources.push({
      ...
      label: clean(record["Primary Source Label"]) || "Clippers reconciled audit workbook",
      sourceTradeId,
      sourceTeam: "los-angeles-clippers",
      privateOnly: true,
    });
  ...
  return {
    ...
    slug: `los-angeles-clippers-trade-${tradeDate}-${sourceTradeId.split("-").at(-1)}`,
    league: "nba",
    tradeDate,
    date: tradeDate,
    ...
    sourceTeamLabels: uniqueSorted([
      clean(record["Franchise Label"]) || "Los Angeles Clippers",
      clean(record["Partner Team(s)"]),
    ]),
    sourceTeamSlugs: uniqueSorted(["los-angeles-clippers", sourcePartner]),
    assetsReceived: byReceived,
    assetsSent: bySent,
    assetsSentByTeam: bySent,
    assetLedger: assets,
    sourceTeams: ["los-angeles-clippers"],
    perspectives: { "los-angeles-clippers": perspective },
    ...
    importReviewStatus: "private-imported-los-angeles-clippers-phase-15h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
    ...
    perspectiveReconciliations: [
      {
        sourceBatchId: "los-angeles-clippers-phase-15h",
        sourceTradeId,
        packageId: `los-angeles-clippers-phase-15h-${sourceTradeId}`,
...
function createPlayerShell(shell, importedAt) {
  ...
  return {
    ...
    publishStatus: "private",
    reviewStatus: "manual-review",
    importReviewStatus: "private-shell-imported-los-angeles-clippers-phase-15h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
...
function assetMatchesClippersSide(asset, side) {
  ...
  if (side === "received") {
    return clean(asset.toTeam) === "los-angeles-clippers" || possibleTo.includes("los-angeles-clippers");
  }
  return clean(asset.fromTeam) === "los-angeles-clippers" || possibleFrom.includes("los-angeles-clippers");
}
function resolveRelationshipAssetReference(trade, relationship, player) {
  ...
  const candidates = assets
    .map((asset) => ({
      ...
      sideMatch: assetMatchesClippersSide(asset, clean(relationship.Side)),
    }))
  ...
  return {
    assetId: `phase15h-perspective-asset-${sha256(
      [
...
const PHASE15H_EXISTING_PLAYER_TARGET_OVERRIDES = new Map();
const PHASE15H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES = new Map([
  ["los-angeles-clippers:LAC-1977-0027:received:001:identity:01:player:nba-player-george-johnson-76fb1237b7", "nba-player-george-johnson-thomas-46726b5067"],
  ["los-angeles-clippers:LAC-1977-0033:sent:001:identity:01:player:nba-player-george-johnson-76fb1237b7", "nba-player-george-johnson-thomas-46726b5067"],
]);
const PHASE15H_REDUNDANT_READY_SHELL_IDS = new Set();
const PHASE15H_REDUNDANT_READY_RELATIONSHIP_IDS = new Set();
function phase15HCorrectRelationship(row) {
  const relationshipId = clean(row["Relationship Edge Key"]);
  if (PHASE15H_REDUNDANT_READY_RELATIONSHIP_IDS.has(relationshipId)) return null;
  const frozenTarget = clean(row["Target Player Key"]);
  const correctedTarget =
    PHASE15H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES.get(relationshipId) ??
    PHASE15H_EXISTING_PLAYER_TARGET_OVERRIDES.get(frozenTarget) ??
    frozenTarget;
  ...
  return {
    ...
    "Proposed Player ID": "",
    "Identity Status": "phase15h-explicit-existing-player-resolution",
  };
}
...
assert(recordsDocument.result === "PASS" && recordsDocument.phase === "15B" && Array.isArray(recordsDocument.records), "Invalid Clippers reconciled records document.");
assert(partitionDocument.result === "PASS" && partitionDocument.phase === "15F", "Invalid Phase 15F partition.");
assert(partitionDocument.hashes.semanticPartitionSha256 === args["expected-partition-internal-sha256"], "Phase 15F semantic partition hash drifted.");
assert(Array.isArray(trades) && Array.isArray(players) && Array.isArray(teams), "A repository store is invalid.");
assert(contractBytes.length > 0, "Phase 15H contract is empty.");

for (const [actual, expected, label] of [
  [partitionDocument.counts.sourceRows, 202, "source rows"],
  [partitionDocument.counts.importReadyPackages, 162, "ready packages"],
  [partitionDocument.counts.heldPackages, 20, "held packages"],
  [partitionDocument.counts.structuralEvidenceExclusions, 20, "structural/evidence exclusions"],
  [partitionDocument.counts.canonicalPerspectiveAppendPreviews, 79, "perspective appends"],
  [partitionDocument.counts.canonicalCreatePreviews, 83, "canonical creates"],
  [partitionDocument.counts.readyRequiredPlayerShells, 82, "ready player shells"],
  [partitionDocument.counts.heldOnlyPlayerShells, 14, "held-only player shells"],
  [partitionDocument.counts.readyRelationshipEdges, 464, "ready relationships"],
  [partitionDocument.counts.heldRelationshipEdges, 87, "held relationships"],
  [partitionDocument.counts.readyPublicCandidatePackages, 40, "ready public candidates"],
]) assert(actual === expected, `Partition ${label} drifted: ${actual} !== ${expected}.`);

assert(readyRows.length === 162, `Expected 162 ready rows, found ${readyRows.length}.`);
assert(heldRows.length === 20, `Expected 20 held rows, found ${heldRows.length}.`);
assert(structuralRows.length === 20, `Expected 20 structural rows, found ${structuralRows.length}.`);
assert(readyShells.length === 82, `Expected 82 ready player shells, found ${readyShells.length}.`);
assert(heldShells.length === 14, `Expected 14 held-only player shells, found ${heldShells.length}.`);
assert(readyRelationshipsInput.length === 464, `Expected 464 ready relationships, found ${readyRelationshipsInput.length}.`);
assert(heldRelationshipsInput.length === 87, `Expected 87 held relationships, found ${heldRelationshipsInput.length}.`);
assert(frozenDependencies.length === 649, `Expected 649 dependency seeds, found ${frozenDependencies.length}.`);
...
const readyPackages = readyRows.map((row) => ({
  ...
  importAction: clean(row["Phase 15F Canonical Write"]) === "CANONICAL_CREATE_PREVIEW" ? "canonical-create" : "perspective-append",
...
assert(readyPackages.filter((item) => item.importAction === "canonical-create").length === 83, "Ready canonical-create count drifted.");
assert(readyPackages.filter((item) => item.importAction === "perspective-append").length === 79, "Ready perspective-append count drifted.");
assert(new Set(readyPackages.map((item) => item.tradeId)).size === 162, "Ready source IDs are not unique.");
assert(new Set(heldPackages.map((item) => item.tradeId)).size === 20, "Held source IDs are not unique.");
assert(new Set(exclusions.map((item) => item.tradeId)).size === 20, "Excluded source IDs are not unique.");
assert(JSON.stringify([...partitionDocument.readyTradeIds].sort()) === JSON.stringify(readyPackages.map((item) => item.tradeId).sort()), "Phase 15F ready IDs do not match ready-package CSV.");
assert(JSON.stringify([...partitionDocument.heldTradeIds].sort()) === JSON.stringify(heldPackages.map((item) => item.tradeId).sort()), "Phase 15F held IDs do not match held-package CSV.");
assert(JSON.stringify([...partitionDocument.structuralTradeIds].sort()) === JSON.stringify(exclusions.map((item) => item.tradeId).sort()), "Phase 15F structural IDs do not match structural CSV.");
...
if (existingReceipt) {
  assert(existingReceipt.result === "PASS" && existingReceipt.phase === "15H", "Existing Phase 15H receipt is invalid.");
  ...
  assert(existingReceipt.readyPackages === 162, "Replay ready-package count drifted.");
  assert(existingReceipt.canonicalTradesCreated === 83, "Replay create count drifted.");
  assert(existingReceipt.perspectivesAppended === 79, "Replay append count drifted.");
  assert(existingReceipt.playerShellsCreated === 82, "Replay shell count drifted.");
  assert(existingReceipt.relationshipReferencesAdded === 464, "Replay relationship count drifted.");
...
assert(trades.length === 1931, `Expected 1,931 pre-import trades, found ${trades.length}.`);
assert(players.length === 2783, `Expected 2,783 pre-import players, found ${players.length}.`);
...
assert(sourceRecords.size === 202, "Clippers source Trade IDs are not unique.");
...
for (const row of readyRows) {
  assert(clean(row["Phase 15F Partition"]) === "IMPORT_READY", `${row["Trade ID"]}: ready partition marker drifted.`);
...
assert(new Set(frozenShells.map((shell) => clean(shell["Proposed Player ID"]))).size === 96, "Frozen shell IDs are not unique.");
assert(new Set(frozenRelationships.map((row) => clean(row["Relationship Edge Key"]))).size === 551, "Frozen relationship IDs are not unique.");
...
for (const shell of readyShells) {
  const id = clean(shell["Proposed Player ID"]);
  const resolvedId = PHASE15H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id);
  if (resolvedId) {
  ...
  if (PHASE15H_REDUNDANT_READY_SHELL_IDS.has(id)) {
...
assert(importedPlayerIds.length === 82, `Expected 82 player-shell creates, found ${importedPlayerIds.length}.`);
assert(resolvedReadyShellIds.length === 0, "Expected zero ready shells resolved to existing players.");
...
assert(readyRelationshipsFrozen.length === 464, `Expected 464 frozen ready relationship previews, found ${readyRelationshipsFrozen.length}.`);
const readyRelationships = readyRelationshipsFrozen.map(phase15HCorrectRelationship).filter(Boolean);
assert(readyRelationships.length === 464, `Expected 464 writable ready relationships, found ${readyRelationships.length}.`);
const readyDependencies = frozenDependencies.filter((row) => readySourceIds.has(clean(row["Trade ID"])));
assert(readyDependencies.length === 540, `Expected 540 ready dependency seeds, found ${readyDependencies.length}.`);
...
for (const packageRecord of readyPackages) {
  ...
  if (packageRecord.importAction === "canonical-create") {
    ...
    assert(uniqueSorted(existing.teams ?? []).includes("los-angeles-clippers"), `${sourceId}: perspective target does not include Clippers.`);
    protectedAppendProjectionHashes[targetId] = sha256(canonicalJson(immutableTradeProjection(existing)));
    trade = appendClippersPerspective(existing, record, args["imported-at"]);
  ...
  for (const relationship of packageRelationships) {
    ...
    const reference = {
      ...
      packageId: `los-angeles-clippers-phase-15h-${sourceId}`,
      sourceTeam: "los-angeles-clippers",
...
assert(importedCanonicalIds.length === 83, "Canonical-create count drifted.");
assert(appendedPerspectiveIds.length === 79, "Perspective-append count drifted.");
assert(relationshipIds.length === 464, "Relationship count drifted.");
assert(tradeMap.size === 2014, `Expected 1,931 post-import trades, found ${tradeMap.size}.`);
assert(playerMap.size === 2865, `Expected 2,783 post-import players, found ${playerMap.size}.`);
assert(matchedExistingAssetReferences + syntheticPerspectiveAssetReferences === 464, "Relationship asset-reference accounting drifted.");
...
for (const packageRecord of readyPackages) {
  ...
  assert(sourcePerspectiveCount(trade, "los-angeles-clippers") === 1, `${sourceId}: Clippers perspective count is not exactly one.`);
...
const receipt = {
  ...
  batchId: "los-angeles-clippers-phase-15h",
  startingHead: args["starting-head"],
  importedAt: args["imported-at"],
  sourceHashes: {
    phase15BRecordsSha256: sha256(recordsBytes),
    phase15FPartitionSha256: sha256(partitionBytes),
    phase15FSemanticPartitionSha256: partitionDocument.hashes.semanticPartitionSha256,
  ...
  redundantReadyRelationshipEdgesExcluded: PHASE15H_REDUNDANT_READY_RELATIONSHIP_IDS.size,
  ...
  readyShellsResolvedToExistingPlayerIds: uniqueSorted(resolvedReadyShellIds.map((id) => PHASE15H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id))),
  redundantReadyShellIds: uniqueSorted(redundantReadyShellIds),
  explicitPlayerTargetCorrections: Object.fromEntries(PHASE15H_EXISTING_PLAYER_TARGET_OVERRIDES),
  explicitRelationshipTargetCorrections: Object.fromEntries(PHASE15H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES),
...
await atomicWrite(args["trades-json"], tradeOut, "phase15h-trades");
await atomicWrite(args["players-json"], playerOut, "phase15h-players");
await atomicWrite(receiptPath, receiptOut, "phase15h-receipt");
