...
const BATCH = "minnesota-timberwolves-phase-20h";
...
function winnerFromVerdict(verdict) {
  ...
  if (value.includes("wolves win") || value.includes("wolves edge")) return "Minnesota Timberwolves";
...
function perspectiveGrades(record, partner) {
  const grades = {};
  if (clean(record.wolvesGrade)) grades[TEAM] = clean(record.wolvesGrade);
...
function wolvesPerspective(record, teams) {
...
function appendWolvesPerspective(existingTrade, record, importedAt) {
  const protectedBefore = JSON.stringify(immutableTradeProjection(existingTrade));
  assert(sourcePerspectiveCount(existingTrade, TEAM) === 0, `${record.tradeId}: Minnesota perspective already exists.`);
  const teams = uniqueSorted(existingTrade.teams ?? []);
  assert(teams.includes(TEAM), `${record.tradeId}: target canonical trade does not include Minnesota.`);
  const perspective = wolvesPerspective(record, teams);
  let perspectives;
  if (Array.isArray(existingTrade.perspectives)) {
    ...
    perspectives = {
      ...
      [TEAM]: {
        ...
        grade: clean(record.wolvesGrade),
        verdict: clean(record.finalVerdict),
        summary: clean(record.summary),
        analysis: clean(record.analysis),
  ...
  assert(sourcePerspectiveCount(updated, TEAM) === 1, `${record.tradeId}: Minnesota perspective append count drifted.`);
  return updated;
}
function canonicalIdForSource(sourceTradeId) {
  const match = clean(sourceTradeId).match(/^MINNBA-(\d{4})-(\d{4})$/u);
  assert(match, `Invalid Minnesota source Trade ID: ${sourceTradeId}`);
  return `nba-trade-min-${match[1]}-${match[2]}`;
}
...
function buildNewTrade({ packageRecord, record, dependencyRows, identityRows, relationshipRows, teamRows, importedAt, playerMap }) {
  ...
  assert(teams.length === 2 && teams.includes(TEAM), `${sourceTradeId}: ready team set must contain Minnesota plus one partner.`);
  ...
  if (!sources.length) {
    sources.push({ sourceType:"reconciled_workbook", label:"Minnesota Timberwolves reconciled audit workbook", sourceTradeId, sourceTeam:TEAM, privateOnly:true });
  }
  return {
    id: canonicalId,
    tradeId: canonicalId,
    ...
    slug: `minnesota-timberwolves-trade-${tradeDate}-${sourceTradeId.split("-").at(-1)}`,
    league: "nba",
    tradeDate,
    date: tradeDate,
    ...
    teams,
    sourceTeamLabels: uniqueSorted(["Minnesota Timberwolves", clean(record.partnerTeams)]),
    sourceTeamSlugs,
...
function assetMatchesWolvesSide(asset, side) {
...
function resolveRelationshipAssetReference(trade, relationship, identity, player) {
  ...
  const candidates = assets.map((asset) => ({
    ...
    sideMatch: assetMatchesWolvesSide(asset, clean(relationship.Side)),
  }))
...
assert(partition.result === "PASS" && partition.phase === "20F-R1" && partition.team === TEAM, "Partition metadata invalid.");
assert(clean(partition.semanticPartitionSha256) === args["expected-partition-internal-sha256"], "Partition semantic hash mismatch.");
assert(partition.counts.importReadyPackages === 97 && partition.counts.heldPackages === 23 && partition.counts.structuralEvidenceExclusions === 1, "Partition package counts drifted.");
assert(partition.counts.canonicalPerspectiveAppendPreviews === 59 && partition.counts.canonicalCreatePreviews === 38, "Partition action counts drifted.");
assert(partition.counts.readyRequiredPlayerShells === 35 && partition.counts.heldOnlyPlayerShells === 13, "Partition player shell counts drifted.");
assert(partition.counts.readyRelationshipEdges === 300 && partition.counts.heldRelationshipEdges === 93, "Partition relationship counts drifted.");
assert(partition.counts.readyTeamDependencyOccurrences === 194 && partition.counts.heldTeamDependencyOccurrences === 75, "Partition team counts drifted.");
...
assert(records.length === 121, `Expected 121 records; received ${records.length}.`);
assert(readyPackages.length === 97 && heldPackages.length === 23 && structuralRows.length === 1, "CSV package partition drifted.");
assert(readyShells.length === 35 && heldShells.length === 13, "CSV shell partition drifted.");
assert(readyRelationships.length === 300 && heldRelationships.length === 93, "CSV relationship partition drifted.");
assert(readyTeams.length === 194 && heldTeams.length === 75, "CSV team partition drifted.");
assert(readyIdentities.length === 300 && heldIdentities.length === 93, "CSV identity partition drifted.");
assert(readyPackages.every((row) => clean(row["Phase 20F Partition"]) === "IMPORT_READY"), "Non-ready package entered ready CSV.");
...
assert(heldIdentities.filter((row) => clean(row["Identity Status"]) === "ambiguous-existing-player").length === 0, "Held ambiguous identity count drifted.");
...
assert(Array.isArray(trades) && trades.length === 2249, "Expected 2249 baseline canonical trades.");
assert(Array.isArray(players) && players.length === 3093, "Expected 3093 baseline players.");
...
assert(teamSet.has(TEAM), "Minnesota Timberwolves missing from team registry.");

const recordsById = new Map(records.map((row) => [clean(row.tradeId), row]));
assert(recordsById.size === 121, "Duplicate source Trade IDs.");
...
assert(new Set([...readyIdSet,...heldIdSet,...excludedIdSet]).size === 121, "Ready/held/excluded ID coverage drifted.");
...
assert(createdPlayerIds.length === 35, `Expected 35 player shells created; received ${createdPlayerIds.length}.`);
assert(resolvedExistingPlayerOverrides.length === 0, `Expected 0 exact existing-player overrides; received ${resolvedExistingPlayerOverrides.length}.`);
...
for (const packageRecord of readyPackages.slice().sort((a,b) => Number(a["Source Row"]) - Number(b["Source Row"]))) {
  ...
  if (action === "NEW_CANONICAL_CANDIDATE") {
    ...
    assert(sourcePerspectiveCount(existing, TEAM) === 0, `${sourceId}: perspective target already has Minnesota perspective.`);
    protectedAppendProjectionHashes[targetId] = sha256(canonicalJson(immutableTradeProjection(existing)));
    trade = appendWolvesPerspective(existing, record, args["imported-at"]);
...
assert(importedCanonicalTradeIds.length === 38, `Expected 38 canonical creates; received ${importedCanonicalTradeIds.length}.`);
assert(updatedPerspectiveCanonicalIds.length === 59, `Expected 59 perspective appends; received ${updatedPerspectiveCanonicalIds.length}.`);
...
assert(relationshipIds.length === 300, `Expected 300 relationship references written; received ${relationshipIds.length}.`);
assert(matchedExistingAssetReferences + syntheticPerspectiveAssetReferences === 300, "Asset reference partition does not equal 300.");

const finalTrades = [...tradeMap.values()];
const finalPlayers = [...playerMap.values()];
assert(finalTrades.length === 2287, `Expected 2287 canonical trades; received ${finalTrades.length}.`);
assert(finalPlayers.length === 3128, `Expected 3128 players; received ${finalPlayers.length}.`);
...
const receipt = {
  ...
  preImportCanonicalTrades: 2249,
  preImportPlayers: 3093,
  preImportTeams: 52,
  readyPackages: 97,
  heldPackages: 23,
  structuralEvidenceExclusions: 1,
  canonicalTradesCreated: 38,
  ...
  effectiveReadyTeamDependencies: 194,
