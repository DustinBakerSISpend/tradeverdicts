...
const PHASE14H_HISTORICAL_TEAM_ALIASES = new Map([
  ["minnesota-muskies", "the-floridians"],
  ["carolina-cougars", "spirits-of-st-louis"],
  ["memphis-pros", "memphis-sounds"],
  ["pistons", "detroit-pistons"],
]);
function canonicalTeamSlug(value) {
  const slug = clean(value);
  return PHASE14H_HISTORICAL_TEAM_ALIASES.get(slug) ?? slug;
}

function perspectiveGrades(record, partner) {
  const grades = {};
  const pacers = clean(record["Pacers Grade"]);
  const aggregate = clean(record["Partner Aggregate Grade"]);
  if (pacers) grades["indiana-pacers"] = pacers;
  if (aggregate) {
...
function indianaPerspective(record, teams) {
  const partner = teams.find((team) => team !== "indiana-pacers") ?? null;
  return {
    sourceTeam: "indiana-pacers",
    sourceBatchId: "indiana-pacers-phase-14h",
    sourceTradeId: clean(record["Trade ID"]),
    sourcePerspectiveKey: `indiana-pacers:${clean(record["Trade ID"])}`,
...
function appendIndianaPerspective(existingTrade, record, importedAt) {
  const protectedBefore = JSON.stringify(immutableTradeProjection(existingTrade));
  assert(
    sourcePerspectiveCount(existingTrade, "indiana-pacers") === 0,
    `${record["Trade ID"]}: Indiana perspective already exists.`,
  );
  const teams = uniqueSorted(existingTrade.teams ?? []);
  const perspective = indianaPerspective(record, teams);
  let perspectives;
  if (Array.isArray(existingTrade.perspectives)) {
    ...
    perspectives = {
      ...
      "indiana-pacers": {
        sourceSubmissionId: `indiana-pacers-phase-14h-${clean(record["Trade ID"])}`,
        sourceTradeId: clean(record["Trade ID"]),
        editorialStatus: "private-imported-indiana-pacers-phase-14h",
        grade: clean(record["Pacers Grade"]),
        verdict: clean(record["Final Verdict"]),
        summary: clean(record["Final Trade Summary"]),
        analysis: clean(record["Final Trade Analysis"]),
  ...
  const pacersGrade = clean(record["Pacers Grade"]);
  const partnerGrade = clean(record["Partner Aggregate Grade"]);
  if (pacersGrade) mergedGrades["indiana-pacers"] = pacersGrade;
  ...
  const updated = {
    ...
    perspectiveReconciliations: [
        ...
        : []),
      {
        sourceBatchId: "indiana-pacers-phase-14h",
        sourceTradeId: clean(record["Trade ID"]),
        packageId: `indiana-pacers-phase-14h-${clean(record["Trade ID"])}`,
  ...
  assert(
    sourcePerspectiveCount(updated, "indiana-pacers") === 1,
    `${record["Trade ID"]}: Indiana perspective append count drifted.`,
  );
  return updated;
}
function canonicalIdForSource(sourceTradeId) {
  const match = clean(sourceTradeId).match(/^IND-(\d{4})-(\d{4})$/u);
  assert(match, `Invalid Indiana source Trade ID: ${sourceTradeId}`);
  return `nba-trade-ind-${match[1]}-${match[2]}`;
}
function makeAssetId(sourceTradeId, side, assetIndex, rawAsset, fromTeam, toTeam) {
  return `phase14h-asset-${sha256(
...
function buildNewTrade({ packageRecord, record, dependencyRows, relationshipRows, importedAt, playerMap }) {
  ...
  const teams = uniqueSorted(["indiana-pacers", partner]);
  ...
  const assets = dependencyRows
    ...
    .map((dependency) => {
      ...
      const fromTeam = side === "received" ? partner : "indiana-pacers";
      const toTeam = side === "received" ? "indiana-pacers" : partner;
      ...
      const asset = {
        ...
        direction: side,
        sourceTeam: "indiana-pacers",
        edgeClass: "indiana-source-route",
        ...
        privateOnly: true,
        previewOnly: false,
        auditStatus: "private-imported-indiana-pacers-phase-14h",
      };
      if (linked.length > 0) {
  ...
  if (sources.length === 0) {
    sources.push({
      ...
      label: clean(record["Primary Source Label"]) || "Indiana reconciled audit workbook",
      sourceTradeId,
      sourceTeam: "indiana-pacers",
      privateOnly: true,
    });
  ...
  return {
    ...
    slug: `indiana-pacers-trade-${tradeDate}-${sourceTradeId.split("-").at(-1)}`,
    league: "nba",
    tradeDate,
    date: tradeDate,
    ...
    sourceTeamLabels: uniqueSorted([
      clean(record["Franchise Label"]) || "Indiana Pacers",
      clean(record["Partner Team(s)"]),
    ]),
    sourceTeamSlugs: uniqueSorted(["indiana-pacers", sourcePartner]),
    assetsReceived: byReceived,
    assetsSent: bySent,
    assetsSentByTeam: bySent,
    assetLedger: assets,
    sourceTeams: ["indiana-pacers"],
    perspectives: { "indiana-pacers": perspective },
    ...
    importReviewStatus: "private-imported-indiana-pacers-phase-14h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
    ...
    perspectiveReconciliations: [
      {
        sourceBatchId: "indiana-pacers-phase-14h",
        sourceTradeId,
        packageId: `indiana-pacers-phase-14h-${sourceTradeId}`,
...
function createPlayerShell(shell, importedAt) {
  ...
  return {
    ...
    publishStatus: "private",
    reviewStatus: "manual-review",
    importReviewStatus: "private-shell-imported-indiana-pacers-phase-14h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
...
function assetMatchesIndianaSide(asset, side) {
  ...
  if (side === "received") {
    return clean(asset.toTeam) === "indiana-pacers" || possibleTo.includes("indiana-pacers");
  }
  return clean(asset.fromTeam) === "indiana-pacers" || possibleFrom.includes("indiana-pacers");
}
function resolveRelationshipAssetReference(trade, relationship, player) {
  ...
  const candidates = assets
    .map((asset) => ({
      ...
      sideMatch: assetMatchesIndianaSide(asset, clean(relationship.Side)),
    }))
  ...
  return {
    assetId: `phase14h-perspective-asset-${sha256(
      [
...
const PHASE14H_EXISTING_PLAYER_TARGET_OVERRIDES = new Map([
  ["nba-player-a-j-price", "nba-player-aj-price-fb186d611f"],
  ["nba-player-r-j-hampton", "nba-player-rj-hampton-62cbde2ae5"],
]);
const PHASE14H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES = new Map([
  ["indiana-pacers:IND-1978-0044:received:001:identity:01:player:nba-player-johnny-davis-e6de30e26d", "nba-player-johnny-davis-reginald-15c9ceca6f"],
  ["indiana-pacers:IND-1982-0065:sent:001:identity:01:player:nba-player-johnny-davis-e6de30e26d", "nba-player-johnny-davis-reginald-15c9ceca6f"],
]);
const PHASE14H_REDUNDANT_READY_SHELL_IDS = new Set();
const PHASE14H_REDUNDANT_READY_RELATIONSHIP_IDS = new Set();
function phase14HCorrectRelationship(row) {
  const relationshipId = clean(row["Relationship Edge Key"]);
  if (PHASE14H_REDUNDANT_READY_RELATIONSHIP_IDS.has(relationshipId)) return null;
  const frozenTarget = clean(row["Target Player Key"]);
  const correctedTarget =
    PHASE14H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES.get(relationshipId) ??
    PHASE14H_EXISTING_PLAYER_TARGET_OVERRIDES.get(frozenTarget) ??
    frozenTarget;
  ...
  return {
    ...
    "Proposed Player ID": "",
    "Identity Status": "phase14h-explicit-existing-player-resolution",
  };
}
...
assert(recordsDocument.result === "PASS" && recordsDocument.phase === "14B" && Array.isArray(recordsDocument.records), "Invalid Indiana reconciled records document.");
assert(partitionDocument.result === "PASS" && partitionDocument.phase === "14F", "Invalid Phase 14F partition.");
assert(partitionDocument.hashes.semanticPartitionSha256 === args["expected-partition-internal-sha256"], "Phase 14F semantic partition hash drifted.");
assert(Array.isArray(trades) && Array.isArray(players) && Array.isArray(teams), "A repository store is invalid.");
assert(contractBytes.length > 0, "Phase 14H contract is empty.");

for (const [actual, expected, label] of [
  [partitionDocument.counts.sourceRows, 168, "source rows"],
  [partitionDocument.counts.importReadyPackages, 133, "ready packages"],
  [partitionDocument.counts.heldPackages, 16, "held packages"],
  [partitionDocument.counts.structuralEvidenceExclusions, 19, "structural/evidence exclusions"],
  [partitionDocument.counts.canonicalPerspectiveAppendPreviews, 50, "perspective appends"],
  [partitionDocument.counts.canonicalCreatePreviews, 83, "canonical creates"],
  [partitionDocument.counts.readyRequiredPlayerShells, 85, "ready player shells"],
  [partitionDocument.counts.heldOnlyPlayerShells, 13, "held-only player shells"],
  [partitionDocument.counts.readyRelationshipEdges, 346, "ready relationships"],
  [partitionDocument.counts.heldRelationshipEdges, 57, "held relationships"],
  [partitionDocument.counts.readyPublicCandidatePackages, 31, "ready public candidates"],
]) assert(actual === expected, `Partition ${label} drifted: ${actual} !== ${expected}.`);

assert(readyRows.length === 133, `Expected 133 ready rows, found ${readyRows.length}.`);
assert(heldRows.length === 16, `Expected 16 held rows, found ${heldRows.length}.`);
assert(structuralRows.length === 19, `Expected 19 structural rows, found ${structuralRows.length}.`);
assert(readyShells.length === 85, `Expected 85 ready player shells, found ${readyShells.length}.`);
assert(heldShells.length === 13, `Expected 13 held-only player shells, found ${heldShells.length}.`);
assert(readyRelationshipsInput.length === 346, `Expected 346 ready relationships, found ${readyRelationshipsInput.length}.`);
assert(heldRelationshipsInput.length === 57, `Expected 57 held relationships, found ${heldRelationshipsInput.length}.`);
assert(frozenDependencies.length === 490, `Expected 490 dependency seeds, found ${frozenDependencies.length}.`);
...
const readyPackages = readyRows.map((row) => ({
  ...
  importAction: clean(row["Phase 14F Canonical Write"]) === "CANONICAL_CREATE_PREVIEW" ? "canonical-create" : "perspective-append",
...
assert(readyPackages.filter((item) => item.importAction === "perspective-append").length === 50, "Ready perspective-append count drifted.");
assert(new Set(readyPackages.map((item) => item.tradeId)).size === 133, "Ready source IDs are not unique.");
assert(new Set(heldPackages.map((item) => item.tradeId)).size === 16, "Held source IDs are not unique.");
assert(new Set(exclusions.map((item) => item.tradeId)).size === 19, "Excluded source IDs are not unique.");
assert(JSON.stringify([...partitionDocument.readyTradeIds].sort()) === JSON.stringify(readyPackages.map((item) => item.tradeId).sort()), "Phase 14F ready IDs do not match ready-package CSV.");
assert(JSON.stringify([...partitionDocument.heldTradeIds].sort()) === JSON.stringify(heldPackages.map((item) => item.tradeId).sort()), "Phase 14F held IDs do not match held-package CSV.");
assert(JSON.stringify([...partitionDocument.structuralTradeIds].sort()) === JSON.stringify(exclusions.map((item) => item.tradeId).sort()), "Phase 14F structural IDs do not match structural CSV.");
...
if (existingReceipt) {
  assert(existingReceipt.result === "PASS" && existingReceipt.phase === "14H", "Existing Phase 14H receipt is invalid.");
  ...
  assert(existingReceipt.readyPackages === 133, "Replay ready-package count drifted.");
  ...
  assert(existingReceipt.playerShellsCreated === 83, "Replay shell count drifted.");
  assert(existingReceipt.relationshipReferencesAdded === 346, "Replay relationship count drifted.");
...
assert(trades.length === 1848, `Expected 1,848 pre-import trades, found ${trades.length}.`);
assert(players.length === 2700, `Expected 2,700 pre-import players, found ${players.length}.`);
...
assert(sourceRecords.size === 168, "Indiana source Trade IDs are not unique.");
...
for (const row of readyRows) {
  assert(clean(row["Phase 14F Partition"]) === "IMPORT_READY", `${row["Trade ID"]}: ready partition marker drifted.`);
...
assert(new Set(frozenShells.map((shell) => clean(shell["Proposed Player ID"]))).size === 98, "Frozen shell IDs are not unique.");
assert(new Set(frozenRelationships.map((row) => clean(row["Relationship Edge Key"]))).size === 403, "Frozen relationship IDs are not unique.");
...
for (const shell of readyShells) {
  const id = clean(shell["Proposed Player ID"]);
  const resolvedId = PHASE14H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id);
  if (resolvedId) {
  ...
  if (PHASE14H_REDUNDANT_READY_SHELL_IDS.has(id)) {
...
assert(importedPlayerIds.length === 83, `Expected 83 player-shell creates after two explicit punctuation-alias resolutions, found ${importedPlayerIds.length}.`);
assert(resolvedReadyShellIds.length === 2, "Expected two ready shells resolved to existing players.");
...
assert(readyRelationshipsFrozen.length === 346, `Expected 346 frozen ready relationship previews, found ${readyRelationshipsFrozen.length}.`);
const readyRelationships = readyRelationshipsFrozen.map(phase14HCorrectRelationship).filter(Boolean);
assert(readyRelationships.length === 346, `Expected 346 writable ready relationships, found ${readyRelationships.length}.`);
const readyDependencies = frozenDependencies.filter((row) => readySourceIds.has(clean(row["Trade ID"])));
assert(readyDependencies.length === 413, `Expected 413 ready dependency seeds, found ${readyDependencies.length}.`);
...
for (const packageRecord of readyPackages) {
  ...
  if (packageRecord.importAction === "canonical-create") {
    ...
    assert(uniqueSorted(existing.teams ?? []).includes("indiana-pacers"), `${sourceId}: perspective target does not include Indiana.`);
    protectedAppendProjectionHashes[targetId] = sha256(canonicalJson(immutableTradeProjection(existing)));
    trade = appendIndianaPerspective(existing, record, args["imported-at"]);
  ...
  for (const relationship of packageRelationships) {
    ...
    const reference = {
      ...
      packageId: `indiana-pacers-phase-14h-${sourceId}`,
      sourceTeam: "indiana-pacers",
...
assert(relationshipIds.length === 346, "Relationship count drifted.");
assert(tradeMap.size === 1931, `Expected 1,931 post-import trades, found ${tradeMap.size}.`);
assert(playerMap.size === 2783, `Expected 2,783 post-import players, found ${playerMap.size}.`);
assert(matchedExistingAssetReferences + syntheticPerspectiveAssetReferences === 346, "Relationship asset-reference accounting drifted.");
...
for (const packageRecord of readyPackages) {
  ...
  assert(sourcePerspectiveCount(trade, "indiana-pacers") === 1, `${sourceId}: Indiana perspective count is not exactly one.`);
...
const receipt = {
  ...
  batchId: "indiana-pacers-phase-14h",
  startingHead: args["starting-head"],
  importedAt: args["imported-at"],
  sourceHashes: {
    phase14BRecordsSha256: sha256(recordsBytes),
    phase14FPartitionSha256: sha256(partitionBytes),
    phase14FSemanticPartitionSha256: partitionDocument.hashes.semanticPartitionSha256,
  ...
  redundantReadyRelationshipEdgesExcluded: PHASE14H_REDUNDANT_READY_RELATIONSHIP_IDS.size,
  ...
  readyShellsResolvedToExistingPlayerIds: uniqueSorted(resolvedReadyShellIds.map((id) => PHASE14H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id))),
  redundantReadyShellIds: uniqueSorted(redundantReadyShellIds),
  explicitPlayerTargetCorrections: Object.fromEntries(PHASE14H_EXISTING_PLAYER_TARGET_OVERRIDES),
  explicitRelationshipTargetCorrections: Object.fromEntries(PHASE14H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES),
...
await atomicWrite(args["trades-json"], tradeOut, "phase14h-trades");
await atomicWrite(args["players-json"], playerOut, "phase14h-players");
await atomicWrite(receiptPath, receiptOut, "phase14h-receipt");
