...
function workbookRecord(record) {
  return {
    ...
    "Grizzlies Grade": record.grizzliesGrade,
...
function perspectiveGrades(record, partner) {
  const grades = {};
  const grizzlies = clean(record["Grizzlies Grade"]);
  const aggregate = clean(record["Partner Aggregate Grade"]);
  if (grizzlies) grades["memphis-grizzlies"] = grizzlies;
  if (aggregate) {
...
function grizzliesPerspective(record, teams) {
  const partner = teams.find((team) => team !== "memphis-grizzlies") ?? null;
  return {
    sourceTeam: "memphis-grizzlies",
    sourceBatchId: "memphis-grizzlies-phase-17h",
    sourceTradeId: clean(record["Trade ID"]),
    sourcePerspectiveKey: `memphis-grizzlies:${clean(record["Trade ID"])}`,
...
function appendGrizzliesPerspective(existingTrade, record, importedAt) {
  const protectedBefore = JSON.stringify(immutableTradeProjection(existingTrade));
  assert(
    sourcePerspectiveCount(existingTrade, "memphis-grizzlies") === 0,
    `${record["Trade ID"]}: Grizzlies perspective already exists.`,
  );
  const teams = uniqueSorted(existingTrade.teams ?? []);
  const perspective = grizzliesPerspective(record, teams);
  let perspectives;
  if (Array.isArray(existingTrade.perspectives)) {
    ...
    perspectives = {
      ...
      "memphis-grizzlies": {
        sourceSubmissionId: `memphis-grizzlies-phase-17h-${clean(record["Trade ID"])}`,
        sourceTradeId: clean(record["Trade ID"]),
        editorialStatus: "private-imported-memphis-grizzlies-phase-17h",
        grade: clean(record["Grizzlies Grade"]),
        verdict: clean(record["Final Verdict"]),
        summary: clean(record["Final Trade Summary"]),
        analysis: clean(record["Final Trade Analysis"]),
  ...
  const grizzliesGrade = clean(record["Grizzlies Grade"]);
  const partnerGrade = clean(record["Partner Aggregate Grade"]);
  if (grizzliesGrade) mergedGrades["memphis-grizzlies"] = grizzliesGrade;
  ...
  const updated = {
    ...
    perspectiveReconciliations: [
        ...
        : []),
      {
        sourceBatchId: "memphis-grizzlies-phase-17h",
        sourceTradeId: clean(record["Trade ID"]),
        packageId: `memphis-grizzlies-phase-17h-${clean(record["Trade ID"])}`,
  ...
  assert(
    sourcePerspectiveCount(updated, "memphis-grizzlies") === 1,
    `${record["Trade ID"]}: Grizzlies perspective append count drifted.`,
  );
  return updated;
}
function canonicalIdForSource(sourceTradeId) {
  const match = clean(sourceTradeId).match(/^MEM-(\d{4})-(\d{4})$/u);
  assert(match, `Invalid Grizzlies source Trade ID: ${sourceTradeId}`);
  return `nba-trade-mem-${match[1]}-${match[2]}`;
}
function makeAssetId(sourceTradeId, side, assetIndex, rawAsset, fromTeam, toTeam) {
  return `phase17h-asset-${sha256(
...
function buildNewTrade({ packageRecord, record, dependencyRows, relationshipRows, teamRows, importedAt, playerMap }) {
  ...
  assert(teams.length === 2 && teams.includes("memphis-grizzlies"), `${sourceTradeId}: resolved team set must contain Grizzlies plus one partner.`);
  const partner = teams.find((team) => team !== "memphis-grizzlies");
  ...
  const assets = dependencyRows
    ...
    .map((dependency) => {
      ...
      const fromTeam = side === "received" ? partner : "memphis-grizzlies";
      const toTeam = side === "received" ? "memphis-grizzlies" : partner;
      ...
      const asset = {
        ...
        direction: side,
        sourceTeam: "memphis-grizzlies",
        edgeClass: "grizzlies-source-route",
        ...
        privateOnly: true,
        previewOnly: false,
        auditStatus: "private-imported-memphis-grizzlies-phase-17h",
      };
      if (linked.length > 0) {
  ...
  if (sources.length === 0) {
    sources.push({
      ...
      label: clean(record["Primary Source Label"]) || "Grizzlies reconciled audit workbook",
      sourceTradeId,
      sourceTeam: "memphis-grizzlies",
      privateOnly: true,
    });
  ...
  return {
    ...
    slug: `memphis-grizzlies-trade-${tradeDate}-${sourceTradeId.split("-").at(-1)}`,
    league: "nba",
    tradeDate,
    date: tradeDate,
    ...
    sourceTeamLabels: uniqueSorted([
      clean(record["Franchise Label"]) || "Memphis Grizzlies",
    ...
    assetLedger: assets,
    sourceTeams: ["memphis-grizzlies"],
    perspectives: { "memphis-grizzlies": perspective },
    ...
    importReviewStatus: "private-imported-memphis-grizzlies-phase-17h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
    ...
    perspectiveReconciliations: [
      {
        sourceBatchId: "memphis-grizzlies-phase-17h",
        sourceTradeId,
        packageId: `memphis-grizzlies-phase-17h-${sourceTradeId}`,
...
function createPlayerShell(shell, importedAt) {
  ...
  return {
    ...
    publishStatus: "private",
    reviewStatus: "manual-review",
    importReviewStatus: "private-shell-imported-memphis-grizzlies-phase-17h",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
...
function assetMatchesGrizzliesSide(asset, side) {
  ...
  if (side === "received") {
    return clean(asset.toTeam) === "memphis-grizzlies" || possibleTo.includes("memphis-grizzlies");
  }
  return clean(asset.fromTeam) === "memphis-grizzlies" || possibleFrom.includes("memphis-grizzlies");
}
function resolveRelationshipAssetReference(trade, relationship, player) {
  ...
  if (PHASE17H_FORCE_SYNTHETIC_RELATIONSHIP_IDS.has(relationshipId)) {
    return {
      assetId: `phase17h-perspective-asset-${sha256(
        [
  ...
  const candidates = assets
    .map((asset) => ({
      ...
      sideMatch: assetMatchesGrizzliesSide(asset, clean(relationship.Side)),
    }))
...
const PHASE17H_EXISTING_PLAYER_TARGET_OVERRIDES = new Map([
  ["nba-player-dj-kennedy", "nba-player-d-j-kennedy"],
]);
const PHASE17H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES = new Map();
const PHASE17H_FORCE_SYNTHETIC_RELATIONSHIP_IDS = new Set();
const PHASE17H_REDUNDANT_READY_SHELL_IDS = new Set();
const PHASE17H_REDUNDANT_READY_RELATIONSHIP_IDS = new Set();
function phase17HCorrectRelationship(row) {
  const relationshipId = clean(row["Relationship Edge Key"]);
  if (PHASE17H_REDUNDANT_READY_RELATIONSHIP_IDS.has(relationshipId)) return null;
  const frozenTarget = clean(row["Target Player Key"]);
  const correctedTarget =
    PHASE17H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES.get(relationshipId) ??
    PHASE17H_EXISTING_PLAYER_TARGET_OVERRIDES.get(frozenTarget) ??
    frozenTarget;
  ...
  return {
    ...
    "Proposed Player ID": "",
    "Identity Status": "phase17h-explicit-existing-player-resolution",
  };
}
...
assert(recordsDocument.result === "PASS" && recordsDocument.phase === "17B" && Array.isArray(recordsDocument.records), "Invalid Memphis reconciled records document.");
assert(partitionDocument.result === "PASS" && partitionDocument.phase === "17F", "Invalid Phase 17F partition.");
assert(partitionDocument.hashes.semanticPartitionSha256 === args["expected-partition-internal-sha256"], "Phase 17F semantic partition hash drifted.");
assert(Array.isArray(trades) && Array.isArray(players) && Array.isArray(teams), "A repository store is invalid.");
assert(contractBytes.length > 0, "Phase 17H contract is empty.");

for (const [actual, expected, label] of [
  [partitionDocument.counts.sourceRows, 126, "source rows"],
  [partitionDocument.counts.importReadyPackages, 93, "ready packages"],
  [partitionDocument.counts.heldPackages, 21, "held packages"],
  [partitionDocument.counts.structuralEvidenceExclusions, 12, "structural/evidence exclusions"],
  [partitionDocument.counts.canonicalPerspectiveAppendPreviews, 40, "perspective appends"],
  [partitionDocument.counts.canonicalCreatePreviews, 53, "canonical creates"],
  [partitionDocument.counts.readyRequiredPlayerShells, 59, "ready player shells"],
  [partitionDocument.counts.heldOnlyPlayerShells, 17, "held-only player shells"],
  [partitionDocument.counts.readyRelationshipEdges, 281, "ready relationships"],
  [partitionDocument.counts.heldRelationshipEdges, 101, "held relationships"],
  [partitionDocument.counts.readyTeamDependencyOccurrences, 186, "ready team dependencies"],
  [partitionDocument.counts.heldTeamDependencyOccurrences, 69, "held team dependencies"],
  [partitionDocument.counts.readyPublicCandidatePackages, 39, "ready public candidates"],
]) assert(actual === expected, `Partition ${label} drifted: ${actual} !== ${expected}.`);

assert(readyRows.length === 93, `Expected 93 ready rows, found ${readyRows.length}.`);
assert(heldRows.length === 21, `Expected 21 held rows, found ${heldRows.length}.`);
assert(structuralRows.length === 12, `Expected 12 structural rows, found ${structuralRows.length}.`);
assert(readyShells.length === 59, `Expected 59 ready player shells, found ${readyShells.length}.`);
assert(heldShells.length === 17, `Expected 17 held-only player shells, found ${heldShells.length}.`);
assert(readyRelationshipsInput.length === 281, `Expected 281 ready relationships, found ${readyRelationshipsInput.length}.`);
assert(heldRelationshipsInput.length === 101, `Expected 101 held relationships, found ${heldRelationshipsInput.length}.`);
assert(readyTeamDependencies.length === 186, `Expected 186 ready team dependencies, found ${readyTeamDependencies.length}.`);
assert(heldTeamDependencies.length === 69, `Expected 69 held team dependencies, found ${heldTeamDependencies.length}.`);
assert(readyDependencies.length === 328, `Expected 328 ready dependency seeds, found ${readyDependencies.length}.`);
assert(heldDependencies.length === 116, `Expected 116 held dependency seeds, found ${heldDependencies.length}.`);
assert(frozenDependencies.length === 444, `Expected 444 dependency seeds, found ${frozenDependencies.length}.`);
...
const readyPackages = readyRows.map((row) => ({
  ...
  importAction: clean(row["Phase 17F Canonical Write"]) === "CANONICAL_CREATE_PREVIEW" ? "canonical-create" : "perspective-append",
...
assert(readyPackages.filter((item) => item.importAction === "canonical-create").length === 53, "Ready canonical-create count drifted.");
assert(readyPackages.filter((item) => item.importAction === "perspective-append").length === 40, "Ready perspective-append count drifted.");
assert(new Set(readyPackages.map((item) => item.tradeId)).size === 93, "Ready source IDs are not unique.");
assert(new Set(heldPackages.map((item) => item.tradeId)).size === 21, "Held source IDs are not unique.");
assert(new Set(exclusions.map((item) => item.tradeId)).size === 12, "Excluded source IDs are not unique.");
assert(JSON.stringify([...partitionDocument.readyTradeIds].sort()) === JSON.stringify(readyPackages.map((item) => item.tradeId).sort()), "Phase 17F ready IDs do not match ready-package CSV.");
assert(JSON.stringify([...partitionDocument.heldTradeIds].sort()) === JSON.stringify(heldPackages.map((item) => item.tradeId).sort()), "Phase 17F held IDs do not match held-package CSV.");
assert(JSON.stringify([...partitionDocument.structuralTradeIds].sort()) === JSON.stringify(exclusions.map((item) => item.tradeId).sort()), "Phase 17F structural IDs do not match structural CSV.");
assert(new Set(readyPackages.filter((item) => item.importAction === "perspective-append").map((item) => item.canonicalId)).size === 40, "Ready perspective targets are not unique.");
...
if (existingReceipt) {
  assert(existingReceipt.result === "PASS" && existingReceipt.phase === "17H", "Existing Phase 17H receipt is invalid.");
  ...
  assert(existingReceipt.readyPackages === 93, "Replay ready-package count drifted.");
  assert(existingReceipt.canonicalTradesCreated === 53, "Replay create count drifted.");
  assert(existingReceipt.perspectivesAppended === 40, "Replay append count drifted.");
  assert(existingReceipt.playerShellsCreated === 58, "Replay shell count drifted.");
  assert(existingReceipt.readyShellsResolvedToExistingPlayers === 1, "Replay resolved-shell count drifted.");
  assert(existingReceipt.relationshipReferencesAdded === 281, "Replay relationship count drifted.");
...
assert(trades.length === 2088, `Expected 2,088 pre-import trades, found ${trades.length}.`);
assert(players.length === 2941, `Expected 2,941 pre-import players, found ${players.length}.`);
...
assert(sourceRecords.size === 126, "Grizzlies source Trade IDs are not unique.");
...
for (const row of readyRows) {
  assert(clean(row["Phase 17F Partition"]) === "IMPORT_READY", `${row["Trade ID"]}: ready partition marker drifted.`);
...
assert(new Set(frozenShells.map((shell) => clean(shell["Proposed Player ID"]))).size === 76, "Frozen shell IDs are not unique.");
assert(new Set(frozenRelationships.map((row) => clean(row["Relationship Edge Key"]))).size === 382, "Frozen relationship IDs are not unique.");
assert(new Set([...readyTeamDependencies, ...heldTeamDependencies].map((row) => `${clean(row["Package Key"])}|${clean(row["Team Slug"])}`)).size === 255, "Frozen team dependency keys are not unique.");
...
for (const shell of readyShells) {
  const id = clean(shell["Proposed Player ID"]);
  const resolvedId = PHASE17H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id);
  if (resolvedId) {
  ...
  if (PHASE17H_REDUNDANT_READY_SHELL_IDS.has(id)) {
...
assert(importedPlayerIds.length === 58, `Expected 58 player-shell creates, found ${importedPlayerIds.length}.`);
assert(resolvedReadyShellIds.length === 1, `Expected one ready shell resolved to existing player, found ${resolvedReadyShellIds.length}.`);
...
assert(readyRelationshipsFrozen.length === 281, `Expected 281 frozen ready relationship previews, found ${readyRelationshipsFrozen.length}.`);
const readyRelationshipsWritable = readyRelationshipsFrozen.map(phase17HCorrectRelationship).filter(Boolean);
assert(readyRelationshipsWritable.length === 281, `Expected 281 writable ready relationships, found ${readyRelationshipsWritable.length}.`);
assert(readyDependencies.length === 328, `Expected 328 ready dependency seeds, found ${readyDependencies.length}.`);
...
for (const packageRecord of readyPackages) {
  ...
  assert(resolvedTeamSlugs.length === 2 && resolvedTeamSlugs.includes("memphis-grizzlies"), `${sourceId}: ready package team dependency set drifted.`);
  let trade;
  ...
  if (packageRecord.importAction === "canonical-create") {
    ...
    assert(uniqueSorted(existing.teams ?? []).includes("memphis-grizzlies"), `${sourceId}: perspective target does not include Grizzlies.`);
    ...
    trade = appendGrizzliesPerspective(existing, record, args["imported-at"]);
  ...
  for (const relationship of packageRelationships) {
    ...
    const reference = {
      ...
      packageId: `memphis-grizzlies-phase-17h-${sourceId}`,
      sourceTeam: "memphis-grizzlies",
...
assert(importedCanonicalIds.length === 53, "Canonical-create count drifted.");
assert(appendedPerspectiveIds.length === 40, "Perspective-append count drifted.");
assert(relationshipIds.length === 281, "Relationship count drifted.");
assert(tradeMap.size === 2141, `Expected 2,141 post-import trades, found ${tradeMap.size}.`);
assert(playerMap.size === 2999, `Expected 2,999 post-import players, found ${playerMap.size}.`);
assert(matchedExistingAssetReferences + syntheticPerspectiveAssetReferences === 281, "Relationship asset-reference accounting drifted.");
...
for (const packageRecord of heldPackages) {
  ...
  assert(!appendedPerspectiveIds.includes(targetId), `${sourceId}: held package received a Grizzlies perspective append.`);
  if (targetId) {
...
for (const packageRecord of readyPackages) {
  ...
  assert(sourcePerspectiveCount(trade, "memphis-grizzlies") === 1, `${sourceId}: Grizzlies perspective count is not exactly one.`);
...
const receipt = {
  ...
  redundantReadyRelationshipEdgesExcluded: PHASE17H_REDUNDANT_READY_RELATIONSHIP_IDS.size,
  ...
  existingPerspectiveReviewHolds: heldRows.filter((row) => clean(row["Existing Memphis Perspective"]) === "Yes").length,
  ...
  readyShellsResolvedToExistingPlayerIds: uniqueSorted(resolvedReadyShellIds.map((id) => PHASE17H_EXISTING_PLAYER_TARGET_OVERRIDES.get(id))),
  redundantReadyShellIds: uniqueSorted(redundantReadyShellIds),
  explicitPlayerTargetCorrections: Object.fromEntries(PHASE17H_EXISTING_PLAYER_TARGET_OVERRIDES),
  explicitRelationshipTargetCorrections: Object.fromEntries(PHASE17H_EXPLICIT_RELATIONSHIP_TARGET_OVERRIDES),
...
await atomicWrite(args["trades-json"], tradeOut, "phase17h-trades");
await atomicWrite(args["players-json"], playerOut, "phase17h-players");
await atomicWrite(receiptPath, receiptOut, "phase17h-receipt");
