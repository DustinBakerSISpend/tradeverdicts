...
function correctedShell(shell) {
  ...
  return {
    ...shell,
    originalProposedPlayerId: clean(shell.proposedPlayerId),
    proposedPlayerId: correction.correctedPlayerId,
    displayName: correction.correctedDisplayName,
    phase11GResolvedToExisting: correction.resolveToExisting,
  };
}
function correctedRelationship(relationship) {
  const correction = IDENTITY_CORRECTIONS.get(clean(relationship.targetPlayerId));
  if (!correction) return relationship;
  return {
    ...relationship,
    targetPlayerId: correction.correctedPlayerId,
    proposedPlayerId: relationship.proposedPlayerId
      ? correction.correctedPlayerId
      : relationship.proposedPlayerId,
    displayName: correction.correctedDisplayName,
  };
}
...
const TEAM_NAME_TO_SLUG = new Map([
  ...
  ["Anderson Packers", "anderson-packers"],
  ["Baltimore Bullets (1944–1954)", "baltimore-bullets-original"],
  ...
  ["Denver Nuggets (1948–1950)", "denver-nuggets-original"],
  ["Golden State Warriors", "golden-state-warriors"],
  ["Houston Mavericks (ABA)", "spirits-of-st-louis"],
  ["Houston Rockets", "houston-rockets"],
  ["Indiana Pacers", "indiana-pacers"],
  ["Indianapolis Jets", "indianapolis-olympians"],
...
function routeAsset(sourceRecord, side, rawAsset, partners, explicitRoutingMap) {
  ...
  return side === "received"
    ? {
        ...
        possibleToTeams: [],
        routingStatus: "resolved",
        routingMethod: "phase11g-explicit-routing-map",
      }
    : {
        fromTeam: "detroit-pistons",
        toTeam: configured,
...
function perspectiveGrades(sourceRecord, teams) {
  const grades = {
    "detroit-pistons": clean(sourceRecord["Pistons Grade"]),
  };
  const partnerGrade = clean(sourceRecord["Partner Aggregate Grade"]);
  if (partnerGrade) {
    grades.partnerAggregate = partnerGrade;
    if (teams.length === 2) {
      const partner = teams.find((team) => team !== "detroit-pistons");
      if (partner) grades[partner] = partnerGrade;
    }
  }
  return grades;
}
function detroitPerspective(sourceRecord, teams) {
  return {
    sourceTeam: "detroit-pistons",
    sourceBatchId: "detroit-pistons-phase-11g",
    sourceTradeId: clean(sourceRecord["Trade ID"]),
    sourcePerspectiveKey:
      `detroit-pistons:${clean(sourceRecord["Trade ID"])}`,
...
function appendDetroitPerspective(existingTrade, sourceRecord, importedAt) {
  const protectedBefore = JSON.stringify(immutableTradeProjection(existingTrade));
  assert(
    sourcePerspectiveCount(existingTrade, "detroit-pistons") === 0,
    `${sourceRecord["Trade ID"]}: Detroit perspective already exists.`,
  );

  const teams = uniqueSorted(existingTrade.teams ?? []);
  const perspective = detroitPerspective(sourceRecord, teams);
  let perspectives;
  if (Array.isArray(existingTrade.perspectives)) {
    ...
    perspectives = {
      ...
      "detroit-pistons": {
        sourceSubmissionId:
          `detroit-pistons-phase-11g-${clean(sourceRecord["Trade ID"])}`,
        editorialStatus: "private-imported-detroit-phase-11g",
        grade: clean(sourceRecord["Pistons Grade"]),
        verdict: clean(sourceRecord["Final Verdict"]),
        summary: clean(sourceRecord["Final Trade Summary"]),
        analysis: clean(sourceRecord["Final Trade Analysis"]),
  ...
  const updated = {
    ...
    perspectiveReconciliations: [
        ...
        : []),
      {
        sourceBatchId: "detroit-pistons-phase-11g",
        sourceTradeId: clean(sourceRecord["Trade ID"]),
        packageId: `detroit-pistons-phase-11g-${clean(
  ...
  assert(
    sourcePerspectiveCount(updated, "detroit-pistons") === 1,
    `${sourceRecord["Trade ID"]}: Detroit perspective append count drifted.`,
  );
  return updated;
}
...
function buildNewTrade(packageRecord, relationshipRows, importedAt, playerMap, explicitRoutingMap, routingStats) {
  ...
  const teams = uniqueSorted(["detroit-pistons", ...partners]);
  ...
  for (const [field, side] of [
    ...
    rawAssets.forEach((rawAsset, index) => {
      ...
      if (route.routingMethod === "phase11g-explicit-routing-map") {
      ...
      const asset = {
        ...
        direction: side,
        sourceTeam: "detroit-pistons",
        edgeClass: "detroit-source-route",
        ...
        privateOnly: true,
        previewOnly: false,
        auditStatus: "private-imported-detroit-phase-11g",
      };
  ...
  return {
    ...
    slug: `detroit-pistons-trade-${clean(
      sourceRecord["Trade Date"],
    )}-${clean(sourceRecord["Trade ID"]).split("-").at(-1)}`,
    league: "nba",
    tradeDate: normalizedTradeDate(sourceRecord["Trade Date"]),
    date: normalizedTradeDate(sourceRecord["Trade Date"]),
    seasonLabel: seasonLabel(normalizedTradeDate(sourceRecord["Trade Date"])),
    season: seasonStartYear(normalizedTradeDate(sourceRecord["Trade Date"])),
    teams,
    sourceTeamLabels: uniqueSorted(["Detroit Pistons", ...clean(sourceRecord["Trade Partner(s)"]).split(",").map(clean).filter(Boolean)]),
    assetsReceived: assetsByTeam(teams, assets, "received"),
    assetsSent: assetsByTeam(teams, assets, "sent"),
    assetLedger: assets,
    sourceTeams: ["detroit-pistons"],
    perspectives: [perspective],
    ...
    importReviewStatus: "private-imported-detroit-phase-11g",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
...
function createPlayerShell(shell, importedAt) {
  ...
  return {
    ...
    league: "nba",
    aliases: uniqueSorted((shell.sourceIdentityLabels ?? []).filter((value) => normalize(value) !== normalize(displayName))),
    referenceTypes: [],
    tradeIds: [],
    tradeSlugs: [],
    ...
    publishStatus: "private",
    reviewStatus: "manual-review",
    importReviewStatus: "private-shell-imported-detroit-phase-11g",
    privateOnly: true,
    indexEligible: false,
    adEligible: false,
    publicationReady: false,
...
function assetMatchesDetroitSide(asset, side) {
  ...
  if (side === "received") {
    return (
      clean(asset.toTeam) === "detroit-pistons" ||
      possibleTo.includes("detroit-pistons")
    );
  }
  return (
    clean(asset.fromTeam) === "detroit-pistons" ||
    possibleFrom.includes("detroit-pistons")
  );
}
function resolveRelationshipAssetReference(trade, relationship, player) {
  ...
  const candidates = assets
    .map((asset) => ({
      ...
      sideMatch: assetMatchesDetroitSide(asset, relationship.side),
    }))
  ...
  return {
    assetId: `phase11g-perspective-asset-${sha256(
      [
...
assert(
  partition.result === "PASS" && partition.phase === "11F",
  "Invalid Phase 11F partition.",
);
assert(
  sha256(partitionBytes) === args["expected-partition-file-sha256"],
  "Phase 11F partition file hash drifted.",
);
assert(
  partition.hashes.finalImportPartitionSha256 ===
    args["expected-partition-internal-sha256"],
  "Phase 11F internal partition hash drifted.",
);
...
assert(
  ...
  "Phase 11G explicit routing map is invalid.",
);

for (const [actual, expected, label] of [
  [partition.counts.finalReadyPackages, 258, "ready packages"],
  [partition.counts.remainingHeldPackages, 0, "held packages"],
  [partition.counts.canonicalCreatePackages, 190, "canonical creates"],
  [partition.counts.perspectiveAppendPackages, 68, "perspective appends"],
  [partition.counts.dateCollisionDistinctCreates, 7, "date-collision creates"],
  [partition.counts.linkedOrVoidedExclusions, 20, "linked/voided exclusions"],
  [partition.counts.proposedPlayerShells, 240, "player shells"],
  [partition.counts.relationshipPreviews, 690, "relationship previews"],
]) {
...
if (existingReceipt) {
  assert(
    existingReceipt.result === "PASS" && existingReceipt.phase === "11G",
    "Existing Phase 11G receipt is invalid.",
  );
  assert(trades.length === 1567, "Replay trade count drifted.");
  assert(players.length === 2402, "Replay player count drifted.");
  ...
  assert(
    existingReceipt.readyPackages === 258 &&
      existingReceipt.canonicalTradesCreated === 190 &&
      existingReceipt.perspectivesAppended === 68 &&
      existingReceipt.playerShellsCreated === 238 &&
      existingReceipt.frozenShellsResolvedToExistingPlayers === 2 &&
      existingReceipt.relationshipReferencesAdded === 690,
...
assert(trades.length === 1377, "Expected 1,377 pre-import trades.");
assert(players.length === 2164, "Expected 2,164 pre-import players.");
...
for (const frozenShell of partition.proposedPlayerShells) {
  ...
  const id = clean(shell.proposedPlayerId);
  if (shell.phase11GResolvedToExisting) {
...
assert(importedPlayerIds.length === 238, `Expected 238 new player shells after two frozen identity resolutions, found ${importedPlayerIds.length}.`);
assert(resolvedExistingPlayerIds.length === 2, `Expected two frozen shell resolutions, found ${resolvedExistingPlayerIds.length}.`);
...
for (const packageRecord of partition.finalReadyPackages) {
  ...
  if (packageRecord.importAction === "canonical-create") {
    ...
    trade = appendDetroitPerspective(
      existing,
  ...
  for (const relationship of packageRelationships) {
    ...
    const reference = {
      ...
      sourceTradeId,
      packageId: `detroit-pistons-phase-11g-${sourceTradeId}`,
      sourceTeam: "detroit-pistons",
...
assert(importedCanonicalIds.length === 190, "Canonical-create count drifted.");
assert(appendedPerspectiveIds.length === 68, "Perspective-append count drifted.");
assert(relationshipIds.length === 690, "Relationship count drifted.");
assert(tradeMap.size === 1567, `Expected 1,567 post-import trades, found ${tradeMap.size}.`);
assert(playerMap.size === 2402, `Expected 2,402 post-import players, found ${playerMap.size}.`);
assert(matchedExistingAssetReferences + syntheticPerspectiveAssetReferences === 690, "Relationship asset-reference accounting drifted.");
assert(routingStats.explicitRoutingAssetsApplied >= 0, `Explicit multi-team canonical-create route accounting failed: ${routingStats.explicitRoutingAssetsApplied}.`);
...
for (const packageRecord of partition.finalReadyPackages) {
  ...
  assert(
    ...
    `${sourceId}: Detroit perspective count is not exactly one.`,
  );
...
const receipt = {
  ...
  batchId: "detroit-pistons-phase-11g",
  startingHead: args["starting-head"],
  importedAt: args["imported-at"],
  sourceHashes: {
    phase11FFileSha256: sha256(partitionBytes),
    phase11FInternalPartitionSha256:
      partition.hashes.finalImportPartitionSha256,
...
await atomicWrite(args["trades-json"], tradeOut, "phase11g-trades");
await atomicWrite(args["players-json"], playerOut, "phase11g-players");
await atomicWrite(receiptPath, receiptOut, "phase11g-receipt");
