...
const outPath = path.join(process.cwd(), "audits", "suppress-safe-same-date-team-trade-duplicates-apply-report.json");
...
if ((inspect.safeCount || 0) !== 45) errors.push(`Expected 45 safe rows, found ${inspect.safeCount}`);
if ((inspect.blockedCount || 0) !== 46) errors.push(`Expected 46 blocked rows, found ${inspect.blockedCount}`);
...
if (errors.length) {
  ...
  console.error("");
  console.error("SUPPRESS SAFE SAME-DATE/TEAM TRADE DUPLICATES APPLY BLOCKED");
...
for (const r of safeRows) {
  ...
  suppress.qaNotes = suppress.qaNotes
    ? `${suppress.qaNotes} Suppressed as safe same-date/team duplicate; canonical keeper is ${slugOf(keeper)} (${keeper.id || "no-id"}).`
    : `Suppressed as safe same-date/team duplicate; canonical keeper is ${slugOf(keeper)} (${keeper.id || "no-id"}).`;
...
console.log("");
console.log("SUPPRESS SAFE SAME-DATE/TEAM TRADE DUPLICATES APPLY");
