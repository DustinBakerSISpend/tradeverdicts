...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Packers.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Green Bay Packers";
const SOURCE_TEAM_SLUG = "green-bay-packers";
...
function buildAnalysis(row) {
  const packers = cleanPublicText(row["Packers Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (packers && partner) return `${packers} ${partner}`;
  return packers || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Packers Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Packers Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Packers Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Packers Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshPackersVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshPackersVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Packers trades: ${incomingTrades.length}`);
