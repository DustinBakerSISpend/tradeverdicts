...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Chargers.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Los Angeles Chargers";
const SOURCE_TEAM_SLUG = "los-angeles-chargers";
...
function buildAnalysis(row) {
  const chargers = cleanPublicText(row["Chargers Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (chargers && partner) return `${chargers} ${partner}`;
  return chargers || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Chargers Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Chargers Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Chargers Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Chargers Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshChargersVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshChargersVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Chargers trades: ${incomingTrades.length}`);
