...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Jaguars.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Jacksonville Jaguars";
const SOURCE_TEAM_SLUG = "jacksonville-jaguars";
...
function buildAnalysis(row) {
  const jaguars = cleanPublicText(row["Jaguars Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (jaguars && partner) return `${jaguars} ${partner}`;
  return jaguars || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Jaguars Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Jaguars Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Jaguars Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Jaguars Grade"]),
...
function mergeUniquePerspectives(existingPerspectives = [], incomingPerspectives = []) {
  ...
  for (const rawPerspective of [...existingPerspectives, ...incomingPerspectives]) {
    const perspective = sanitizePerspective(rawPerspective);
...
function stripPublicQaNotes(trade) {
  const cleanTrade = { ...trade };
  delete cleanTrade.qaNotes;

  cleanTrade.perspectives = (cleanTrade.perspectives || []).map(sanitizePerspective);

  return cleanTrade;
}

function mergeTrade(existing, incoming) {
  const cleanExisting = stripPublicQaNotes(existing);
  const cleanIncoming = stripPublicQaNotes(incoming);

  const mergedTeams = Array.from(new Set([...(cleanExisting.teams || []), ...(cleanIncoming.teams || [])]));
  const mergedAssetsReceived = { ...(cleanExisting.assetsReceived || {}) };

  for (const team of Object.keys(cleanIncoming.assetsReceived || {})) {
    mergedAssetsReceived[team] = mergeUniqueAssets(
      mergedAssetsReceived[team] || [],
      cleanIncoming.assetsReceived[team] || []
    );
  }

  const mergedGrades = mergeGradesRefreshIncoming(cleanExisting.grades || {}, cleanIncoming.grades || {});
  const shouldRefreshJaguarsVerdict =
    (cleanIncoming.sourceTeams || []).includes(SOURCE_TEAM_SLUG) && clean(cleanIncoming.verdict);

  const incomingText = [cleanIncoming.summary, cleanIncoming.partnerSummary].filter(Boolean).join(" ");
  const existingHasBlankPublicText = !clean(cleanExisting.summary) || !clean(cleanExisting.analysis);

  return stripPublicQaNotes({
    ...cleanExisting,

    canonicalKey: cleanExisting.canonicalKey || cleanIncoming.canonicalKey,
    dateTeamsKey: cleanExisting.dateTeamsKey || cleanIncoming.dateTeamsKey,
    teams: mergedTeams,
    assetsReceived: mergedAssetsReceived,

    verdict: shouldRefreshJaguarsVerdict ? cleanIncoming.verdict : cleanExisting.verdict,

    grades: mergedGrades,

    sourceTeams: Array.from(new Set([...(cleanExisting.sourceTeams || []), ...(cleanIncoming.sourceTeams || [])])),
    perspectives: mergeUniquePerspectives(cleanExisting.perspectives || [], cleanIncoming.perspectives || []),

    summary:
      existingHasBlankPublicText && cleanIncoming.summary
        ? cleanIncoming.summary
        : cleanExisting.summary,

    partnerSummary:
      existingHasBlankPublicText && cleanIncoming.partnerSummary
        ? cleanIncoming.partnerSummary
        : cleanExisting.partnerSummary,

    analysis:
      existingHasBlankPublicText && (cleanIncoming.analysis || incomingText)
        ? cleanIncoming.analysis || incomingText
        : cleanExisting.analysis,

    confidence:
      cleanExisting.confidence === "high" || cleanIncoming.confidence === "high"
        ? "high"
        : cleanExisting.confidence || cleanIncoming.confidence,

    publishStatus:
      cleanExisting.publishStatus === "ready" || cleanIncoming.publishStatus === "ready"
        ? "ready"
        : cleanExisting.publishStatus || cleanIncoming.publishStatus,
  });
}
...
function main() {
  ...
  const existingTrades = readExistingTrades().map(stripPublicQaNotes);
  ...
  for (const incoming of incomingTrades) {
    ...
    finalTrades.push(stripPublicQaNotes(incoming));
    added++;
  }
  ...
  generatePlayersFile(cleanedFinalTrades);
  fs.writeFileSync(DUPLICATE_REPORT_FILE, JSON.stringify(duplicateReport, null, 2));

  console.log(`Incoming Jaguars trades: ${incomingTrades.length}`);
  ...
  console.log(`Final trade count: ${cleanedFinalTrades.length}`);
