...
function hasBackendLanguage(text) {
  return /publishStatus|qaNotes|provisional|GSC|manually indexed|indexing|hold-review|public import|Status: Ready|Calculated|Verification pending|Retained for TradeVerdicts/i.test(text || "");
}

function hasBadGrammar(text) {
  return /\breceives the edge\b|\bgets the edge\b/i.test(text || "");
}
...
for (const id of ids) {
  ...
  for (const [field, text] of fields) {
    ...
    if (hasBadGrammar(text)) hits.push({ id, field, issue: "bad grammar: edge phrasing", text: text.slice(0, 240) });
...
const outPath = "reports/quality/nfl-bottom-batch-006-public-copy-sanity-scan.txt";

let out = "NFL Bottom Batch 006 Public Copy Sanity Scan\n\n";
