...
import { DATA_PATH, REPORT_DIR, parseArgs, readJson, writeJson, getTrades, getId, safe, readManifest, label, die } from "./nfl-batch-lib.mjs";

const args = parseArgs();
const bottom = Number(args.bottom || args._[0] || 1);
const applyMode = Boolean(args.apply);
const batchLabel = label(bottom);
const decisionPath = path.join(REPORT_DIR, "decisions", `nfl-bottom-batch-${batchLabel}-copy-decisions.json`);
const outJson = path.join(REPORT_DIR, `nfl-bottom-batch-${batchLabel}-copy-${applyMode ? "apply" : "dry-run"}-v1.json`);
const outTxt = path.join(REPORT_DIR, `nfl-bottom-batch-${batchLabel}-copy-${applyMode ? "apply" : "dry-run"}-v1.txt`);
...
if (!fs.existsSync(decisionPath)) {
  const template = {
    ...
    note: "Copy decisions are intentionally explicit. Add records only after reviewing the repair preview.",
    records: [
      {
        id: "EXAMPLE-TRADE-ID",
        summary: "Replacement public summary.",
        partnerSummary: "Replacement public partner summary.",
        analysis: "Replacement public analysis.",
        perspectives: "optional: omit unless you need to replace perspective copy"
      }
    ]
  };
  writeJson(decisionPath, template);
  console.log("");
  console.log(`No copy decisions file found, so I created a template:`);
  console.log(`reports\\quality\\decisions\\nfl-bottom-batch-${batchLabel}-copy-decisions.json`);
  console.log("");
  process.exit(0);
}
...
if (applyMode && blocked === 0) {
  backupPath = path.join(path.dirname(DATA_PATH), `trades.backup-before-bottom-batch-${batchLabel}-copy-${new Date().toISOString().replace(/[:.]/g, "-")}.json`);
...
const txt = `# NFL Bottom Batch ${batchLabel} Copy ${applyMode ? "Apply" : "Dry Run"} v1
...
console.log("");
console.log(`Copy ${applyMode ? "apply" : "dry-run"} complete.`);
console.log(`Open: reports\\quality\\nfl-bottom-batch-${batchLabel}-copy-${applyMode ? "apply" : "dry-run"}-v1.txt`);
