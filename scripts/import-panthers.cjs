...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Panthers.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Carolina Panthers";
const SOURCE_TEAM_SLUG = "carolina-panthers";
...
function buildAnalysis(row) {
  const panthers = cleanPublicText(row["Panthers Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (panthers && partner) return `${panthers} ${partner}`;
  return panthers || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Panthers Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Panthers Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Panthers Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Panthers Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshPanthersVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshPanthersVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Panthers trades: ${incomingTrades.length}`);
