...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Commanders.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Washington Commanders";
const SOURCE_TEAM_SLUG = "washington-commanders";
...
function buildAnalysis(row) {
  const commanders = cleanPublicText(row["Commanders Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (commanders && partner) return `${commanders} ${partner}`;
  return commanders || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Commanders Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Commanders Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Commanders Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Commanders Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshCommandersVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshCommandersVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Commanders trades: ${incomingTrades.length}`);
