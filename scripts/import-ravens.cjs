...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Ravens.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Baltimore Ravens";
const SOURCE_TEAM_SLUG = "baltimore-ravens";
...
function buildAnalysis(row) {
  const ravens = cleanPublicText(row["Ravens Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (ravens && partner) return `${ravens} ${partner}`;
  return ravens || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Ravens Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Ravens Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Ravens Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Ravens Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshRavensVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshRavensVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Ravens trades: ${incomingTrades.length}`);
