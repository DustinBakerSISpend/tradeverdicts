...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Buccaneers.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Tampa Bay Buccaneers";
const SOURCE_TEAM_SLUG = "tampa-bay-buccaneers";
...
function buildAnalysis(row) {
  const source = cleanPublicText(row["Buccaneers Outcome Synopsis"]);
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Buccaneers Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Buccaneers Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Buccaneers Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Buccaneers Grade"]),
...
function main() {
  ...
  console.log(`Incoming Buccaneers trades: ${incomingTrades.length}`);
