...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Giants.xlsx");
...
const SOURCE_TEAM_FALLBACK = "New York Giants";
const SOURCE_TEAM_SLUG = "new-york-giants";
...
function buildAnalysis(row) {
  const giants = cleanPublicText(row["Giants Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (giants && partner) return `${giants} ${partner}`;
  return giants || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Giants Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Giants Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Giants Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Giants Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshGiantsVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshGiantsVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Giants trades: ${incomingTrades.length}`);
