...
const OUT = "src/data/nfl/transaction-merge-score70-apply-report.json";

const trades = JSON.parse(fs.readFileSync(TRADES, "utf8"));
const audit = JSON.parse(fs.readFileSync(AUDIT, "utf8"));

const groups = audit.buckets.likelyDuplicatePlaceholder.filter(g => g.score >= 70);
...
fs.writeFileSync(OUT, JSON.stringify({
  ...
  appliedRule: "transaction-merge likelyDuplicatePlaceholder score >= 70",
  changed,
  changes
}, null, 2));

console.log(`Suppressed ${changed} score-70 transaction placeholder trades`);
