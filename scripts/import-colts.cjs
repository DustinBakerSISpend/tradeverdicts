...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Colts.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Indianapolis Colts";
const SOURCE_TEAM_SLUG = "indianapolis-colts";
const SOURCE_TEAM_SHORT = "Colts";
...
function buildAnalysis(row) {
  const primary = cleanPublicText(row[`${SOURCE_TEAM_SHORT} Outcome Synopsis`]);
...
function buildTrade(row, index) {
  ...
  return {
    ...
    assetsReceived: {
      ...
      [partnerTeam || "unknown-partner"]: partnerReceived,
    },
    ...
    grades: {
      [primaryTeam]: clean(row[`${SOURCE_TEAM_SHORT} Grade`]),
      [partnerTeam || "unknown-partner"]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row[`${SOURCE_TEAM_SHORT} Outcome Synopsis`]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row[`${SOURCE_TEAM_SHORT} Outcome Synopsis`]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row[`${SOURCE_TEAM_SHORT} Grade`]),
...
function main() {
  ...
  console.log(`Incoming Colts trades: ${incomingTrades.length}`);
