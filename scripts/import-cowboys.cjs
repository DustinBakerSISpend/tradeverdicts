...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Cowboys.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Dallas Cowboys";
const SOURCE_TEAM_SLUG = "dallas-cowboys";
...
function buildAnalysis(row) {
  const cowboys = cleanPublicText(row["Cowboys Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (cowboys && partner) return `${cowboys} ${partner}`;
  return cowboys || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Cowboys Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Cowboys Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Cowboys Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Cowboys Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshCowboysVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshCowboysVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Cowboys trades: ${incomingTrades.length}`);
