...
const outPath = path.join(outDir, "ellipsis-same-trade-text-recovery-dry-run.json");
...
function possibleAssetSubstring(fragment, prefix) {
  ...
  const idx = normFragment.indexOf(normPrefix);

  if (idx === -1) return null;

  // We cannot reliably map normalized indexes back to raw indexes,
  // so only use fragments that START with the prefix after trimming obvious prose lead-ins.
  const candidates = [
    fragment,
  ...
  for (const candidate of candidates) {
    if (!candidate.includes("...") && normalize(candidate).startsWith(normPrefix)) {
      return candidate
...
function isSafeCandidate(before, after) {
  ...
  // Guard against accidentally grabbing full prose paragraphs.
  if (after.length > 350) return false;

  // Candidate should look like an asset string, not a sentence about a trade.
  const badPhrases = [
    "the trade",
    "the deal",
    "the move",
...
for (const t of trades) {
  ...
  for (const row of ellipsisAssets) {
    ...
    if (uniqueCandidates.length === 1) {
      ...
      noCandidate.push({
        ...base,
        reason: "No safe same-trade text candidate found"
      });
    }
  }
}
...
console.log("");
console.log("ELLIPSIS SAME-TRADE TEXT RECOVERY DRY RUN");
...
console.log("");
console.log("First 10 unresolved:");
for (const row of [...ambiguous, ...noCandidate].slice(0, 10)) {
  console.log("-".repeat(80));
  console.log(`${row.slug} | ${row.id} | ${row.date} | ${row.team}[${row.assetIndex}]`);
  console.log(`asset: ${row.before}`);
  console.log(`reason: ${row.reason || `${row.candidateCount} candidates`}`);
}
