...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Rams.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Los Angeles Rams";
const SOURCE_TEAM_SLUG = "los-angeles-rams";
...
function buildAnalysis(row) {
  const rams = cleanPublicText(row["Rams Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (rams && partner) return `${rams} ${partner}`;
  return rams || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Rams Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Rams Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Rams Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Rams Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshRamsVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshRamsVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Rams trades: ${incomingTrades.length}`);
