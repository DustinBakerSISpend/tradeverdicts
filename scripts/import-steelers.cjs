...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Steelers.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Pittsburgh Steelers";
const SOURCE_TEAM_SLUG = "pittsburgh-steelers";
...
function buildAnalysis(row) {
  const steelers = cleanPublicText(row["Steelers Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (steelers && partner) return `${steelers} ${partner}`;
  return steelers || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Steelers Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Steelers Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Steelers Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Steelers Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshSteelersVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshSteelersVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Steelers trades: ${incomingTrades.length}`);
