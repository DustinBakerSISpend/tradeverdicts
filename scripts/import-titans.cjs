...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Titans.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Tennessee Titans";
const SOURCE_TEAM_SLUG = "tennessee-titans";
...
function buildAnalysis(row) {
  const titans = cleanPublicText(row["Titans Outcome Synopsis"]);
  const oilers = cleanPublicText(row["Oilers Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  const primary = titans || oilers;

  if (primary && partner) return `${primary} ${partner}`;
  return primary || partner || "";
}

function getPrimaryReceived(row) {
  const titansReceived = splitAssets(row["Titans Received"]);
  if (titansReceived.length > 0) return titansReceived;
  return splitAssets(row["Oilers Received"]);
}

function getPrimarySent(row) {
  const titansSent = splitAssets(row["Titans Sent"]);
  if (titansSent.length > 0) return titansSent;
  return splitAssets(row["Oilers Sent"]);
}

function getPrimaryGrade(row) {
  return clean(row["Titans Grade"]) || clean(row["Oilers Grade"]);
}

function getPrimarySummary(row) {
  return cleanPublicText(row["Titans Outcome Synopsis"]) || cleanPublicText(row["Oilers Outcome Synopsis"]);
}
...
function buildTrade(row, index) {
  ...
  const primaryReceived = getPrimaryReceived(row);
  ...
  const primaryGrade = getPrimaryGrade(row);
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshTitansVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshTitansVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Titans/Oilers trades: ${incomingTrades.length}`);
