...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Broncos.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Denver Broncos";
const SOURCE_TEAM_SLUG = "denver-broncos";
...
function buildAnalysis(row) {
  const broncos = cleanPublicText(row["Broncos Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (broncos && partner) return `${broncos} ${partner}`;
  return broncos || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Broncos Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Broncos Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Broncos Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Broncos Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshBroncosVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshBroncosVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Broncos trades: ${incomingTrades.length}`);
