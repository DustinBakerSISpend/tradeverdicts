...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Eagles.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Philadelphia Eagles";
const SOURCE_TEAM_SLUG = "philadelphia-eagles";
...
function buildAnalysis(row) {
  const eagles = cleanPublicText(row["Eagles Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (eagles && partner) return `${eagles} ${partner}`;
  return eagles || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Eagles Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Eagles Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Eagles Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Eagles Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshEaglesVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshEaglesVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Eagles trades: ${incomingTrades.length}`);
