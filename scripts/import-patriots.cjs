...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Patriots.xlsx");
...
const SOURCE_TEAM_FALLBACK = "New England Patriots";
const SOURCE_TEAM_SLUG = "new-england-patriots";
...
function buildAnalysis(row) {
  const patriots = cleanPublicText(row["Patriots Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (patriots && partner) return `${patriots} ${partner}`;
  return patriots || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Patriots Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Patriots Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Patriots Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Patriots Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshPatriotsVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshPatriotsVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Patriots trades: ${incomingTrades.length}`);
