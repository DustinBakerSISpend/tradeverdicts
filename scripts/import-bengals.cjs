...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Bengals.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Cincinnati Bengals";
const SOURCE_TEAM_SLUG = "cincinnati-bengals";
...
function buildAnalysis(row) {
  const source = cleanPublicText(row["Bengals Outcome Synopsis"]);
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Bengals Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Bengals Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Bengals Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Bengals Grade"]),
...
function main() {
  ...
  console.log(`Incoming Bengals trades: ${incomingTrades.length}`);
