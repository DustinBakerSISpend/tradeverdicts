...
const planPath = path.join(process.cwd(), "audits", "ellipsis-asset-recovery-dry-run.json");
const outPath = path.join(process.cwd(), "audits", "ellipsis-asset-recovery-apply-report.json");
...
if ((plan.ambiguousCount || 0) !== 0) errors.push("Plan unexpectedly contains ambiguous rows.");
...
for (const row of plan.planned || []) {
  ...
  if (String(row.after || "").length <= String(row.before || "").replace("...", "").length) {
    errors.push(`${row.slug}: after-state is not meaningfully longer than before-state`);
    continue;
  }
}
...
if (errors.length) {
  ...
  console.error("");
  console.error("ELLIPSIS ASSET RECOVERY APPLY BLOCKED");
...
if (postErrors.length) {
  ...
  if (postErrors.length > 50) console.error(`...and ${postErrors.length - 50} more`);
  console.error(`Report: ${outPath}`);
  process.exit(1);
}
...
console.log("");
console.log("ELLIPSIS ASSET RECOVERY APPLY");
