...
const planPath = path.join(process.cwd(), "audits", "suppress-covered-historical-generic-dry-run.json");
const outPath = path.join(process.cwd(), "audits", "suppress-covered-historical-generic-apply-report.json");
...
if ((plan.plannedSuppressionCount || 0) !== 6) errors.push(`Expected 6 planned suppressions, found ${plan.plannedSuppressionCount}`);
...
for (const row of plan.planned || []) {
  ...
  if ((trade.teams || []).length <= 2) {
    errors.push(`Not a multi-team artifact anymore: ${row.slug}`);
  }
...
if (errors.length) {
  ...
  console.error("");
  console.error("COVERED HISTORICAL GENERIC APPLY BLOCKED");
...
for (const row of plan.planned || []) {
  ...
  const note = `Suppressed covered historical blended import artifact after coverage check; all player assets appeared on other active pages. Reason: ${row.reason}`;
...
console.log("");
console.log("SUPPRESS COVERED HISTORICAL GENERIC APPLY");
