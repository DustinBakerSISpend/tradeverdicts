...
const planPath = path.join(process.cwd(), "audits", "repair-one-pick-clusters-direct-rows-dry-run.json");
const outPath = path.join(process.cwd(), "audits", "repair-one-pick-clusters-direct-rows-apply-report.json");
...
function cleanSummary(s) {
  return String(s || "")
    .replace(/\b6thround\b/g, "6th round")
    .replace(/\b5thround\b/g, "5th round")
    .replace(/\b4thround\b/g, "4th round")
    .replace(/\ba\s*major\b/g, "a major")
    .replace(/\s+/g, " ")
    .trim();
}
...
if (errors.length) {
  ...
  console.error("");
  console.error("REPAIR ONE-PICK CLUSTERS DIRECT ROWS APPLY BLOCKED");
...
for (const row of plan.planned || []) {
  ...
  const note = `Repaired one-pick cluster contamination; retained direct two-team trade and removed unrelated downstream pick-chain teams/assets. Removed assets archived in apply report. Reason: ${row.reason}`;
...
for (const row of applied) {
  ...
  if (String(trade.summary || "").match(/\b(4th|5th|6th)round\b/)) {
    postErrors.push(`Round-spacing typo still present: ${row.slug}`);
  }
}
...
console.log("");
console.log("REPAIR ONE-PICK CLUSTERS DIRECT ROWS APPLY");
