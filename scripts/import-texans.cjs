...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Texans.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Houston Texans";
const SOURCE_TEAM_SLUG = "houston-texans";
...
function buildAnalysis(row) {
  const texans = cleanPublicText(row["Texans Outcome Synopsis"]);
  const partner = cleanPublicText(row["Partner Outcome Synopsis"]);
  if (texans && partner) return `${texans} ${partner}`;
  return texans || partner || "";
}
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Texans Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Texans Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Texans Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Texans Grade"]),
...
function mergeTrade(existing, incoming) {
  ...
  const shouldRefreshTexansVerdict =
  ...
  return {
    ...
    verdict: shouldRefreshTexansVerdict ? incoming.verdict : existing.verdict,
...
function main() {
  ...
  console.log(`Incoming Texans trades: ${incomingTrades.length}`);
