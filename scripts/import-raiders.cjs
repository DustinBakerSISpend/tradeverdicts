...
const INPUT_FILE = path.join(__dirname, "..", "data-imports", "TradeVerdicts_Raiders.xlsx");
...
const SOURCE_TEAM_FALLBACK = "Las Vegas Raiders";
const SOURCE_TEAM_SLUG = "las-vegas-raiders";
...
function buildAnalysis(row) {
  const source = cleanPublicText(row["Raiders Outcome Synopsis"]);
...
function buildTrade(row, index) {
  ...
  return {
    ...
    grades: {
      [primaryTeam]: clean(row["Raiders Grade"]),
      [partnerTeam]: clean(row["Partner Grade"]),
    },

    confidence: normalizeConfidence(row["Confidence"]),
    summary: cleanPublicText(row["Raiders Outcome Synopsis"]),
    ...
    perspectives: [
      {
        ...
        primaryTeam,
        partnerTeam,
        primarySummary: cleanPublicText(row["Raiders Outcome Synopsis"]),
        partnerSummary: cleanPublicText(row["Partner Outcome Synopsis"]),
        primaryGrade: clean(row["Raiders Grade"]),
...
function main() {
  ...
  console.log(`Incoming Raiders trades: ${incomingTrades.length}`);
